...
Очередь - это структура данных, которая представляет собой упорядоченный набор элементов, где
добавление новых элементов происходит в конец очереди, а доступ и удаление элементов
осуществляются с начала очереди. Примером из реальной жизни является обычная очередь в магазине, или пробка из машин, где все работает по принципу FIFO(first-in -> first-out)
    - 4.1) Функциональная спецификаци
Функциональная спецификация очереди определяет набор операций,
которые могут быть выполнены над очередью. В данном случае, функциональная спецификация
очереди включает следующие операции:
1 Создание пустой очереди (СОЗДАТЬ): Создает новую пустую очередь.
2 Добавление элемента в очередь (В ОЧЕРЕДЬ): Помещает новый элемент в конец очереди.
3 Проверка пустоты очереди (ПУСТО): Проверяет, является ли очередь пустой.
4 Получение первого элемента очереди (ПЕРВЫЙ): Возвращает первый элемент очереди без его
удаления.
5 У даление первого элемента очереди (ИЗ ОЧЕРЕДИ): Извлекает и удаляет первый элемент из очереди,
если она не пуста
Эти операции позволяют работать с очередью, добавлять новые элементы, проверять ее состояние,
извлекать и удалять элементы в соответствии с принципом "первым пришёл - первым ушёл".
Функциональная спецификация также отмечает, что тип элемента очереди обозначается символом T и
может быть любым типом данных, за исключением файлового типа.

    - 4.2) Логическое описание и физическое представление
### 1. Очередь на файле

**Логическое описание:**
Очередь представляет собой упорядоченную коллекцию элементов, где добавление новых элементов происходит в конец очереди, а извлечение элементов осуществляется из начала очереди. Очередь работает по принципу "первым пришёл, первым ушёл" (FIFO).

**Физическое представление:**

* Используется внешний файл для хранения компонентов очереди.
* Для каждой очереди определяется переменная-файл типа "очередь".
* Для долговременных очередей имя файла хранится в заголовке программы.
* Основные операции:

  1. Создать пустую очередь (инициализация файла).
  2. Проверка пустоты файла-очереди.
  3. Добавление элемента в конец очереди (копирование файла в временный файл, добавление элемента, перезапись исходного файла).
  4. Получение первого элемента очереди (начало файла).
  5. Удаление первого элемента очереди (копирование файла без первого элемента в временный файл, перезапись).
* Позволяет сохранять состояние очереди между выполнениями программы.
* Требует дополнительных ресурсов (время на перемотку, временное хранение копий).
```
#include <stdio.h>
#include <stdbool.h>
...
bool push_back(Quene *q, int val){
    ...
    fseek(q->file,((q->head + q->size) % q->max_size) * sizeof(int), SEEK_SET);
...
**Логическое описание:**
Очередь — структура данных, где элементы добавляются в конец и извлекаются из начала очереди (FIFO).

**Физическое представление:**

* Массив используется как сплошной участок памяти.
* Индексные переменные: "голова" (начало очереди) и "хвост" (первый свободный элемент после последнего).
*  Есть несколько стратегий отображения, а именно:

  1. **Стратегия трудоголика:** голова фиксирована, хвост подвижен; при извлечении элементы сдвигаются к началу массива.
  2. **Ленивая очередь:** хвост сдвигается, извлечение занимает постоянное время.
  3. **Кольцевой буфер:** очередь "склеивается" на массиве, позволяя бесконечное движение вперед без сборки мусора.
* Реализация на массиве обычно требует ограничения максимальной длины очереди, а время операций зависит от выбранной стратегии.
...
```
### 3. Очередь на динамических объектах (связанный список)

**Логическое описание:**
Очередь представлена как последовательность элементов, где каждый элемент содержит ссылку на следующий. Первый элемент указывает на второй, второй — на третий и так далее, последний элемент не имеет ссылки на следующий.

**Физическое представление:**

* Используется связанный список; каждый узел содержит данные и ссылку на следующий элемент.
* Для операций необходимо иметь ссылки на начало и конец очереди.
* Новый элемент добавляется в конец очереди через изменение ссылки последнего элемента.
* Извлечение элемента происходит с начала очереди, при этом ссылка на первый элемент обновляется, удаленный элемент освобождается.
* Длина очереди может храниться в переменной, актуализируемой при модификациях.
* Реверсирование очереди может выполняться рекурсивно: элементы извлекаются и помещаются в локальный стек, затем добавляются обратно в обратном порядке.
```
ypedef struct{
    int data;
    struct Node *next;
...
Стек - это структура данных, где элементы добавляются и извлекаются в определенном порядке,
подобно складыванию предметов в вертикальную стопку . Основной принцип стека - "первым пришел,
последним ушел". Чтение из стека разрушает его содержимое. В информатике стеки широко
используются для синтаксического анализа текста, управления локальными переменными, выполнения
рекурсивных процедур и архитектуры микропроцессоров. Примерами стеков в повседневной жизни
являются стопка бумаг , книжная пачка и патроны в магазине автомата. Стеки помогают оптимизировать
и моделировать различные процессы, где необходимо управление порядком и доступом к данным.

    - 5.1) Функциональная спецификация
1. Операция СОЗДАТЬ: создает стек
2. Операция ПУСТО: Возвращает значение "true", если стек пуст, и "false" в противном случае.
3. Операция ГЛУБИНА: Возвращает количество элементов в стеке.
4. Операция В СТЕК: Добавляет элемент T в верхушку стека ST и возвращает обновленный стек
5. Операция ИЗ СТЕКА: У даляет элемент из верхушки стека ST и возвращает обновленный стек
6. Операция ВЕРХ: Возвращает элемент на вершине стека ST.
7. Операция УНИЧТОЖИТЬ: Уничтожает все элементы в стеке, делая его пустым
Замечания:
- Операции ВЕРХ и ИЗ СТЕКА могут быть объединены в одну операцию, возвращающую как элемент,
так и обновленный стек.
- При выборке элемента из верхушки стека и последующем его возвращении в стек получается
исходный стек.
- Последовательность операций В СТЕК и ИЗ СТЕКА позволяет инвертировать порядок элементов в
стеке.
- Непустота стека ограничивает использование определенных операций и свойств

    - 5.2) Логическое описание
верхушка стека - последний введенный элемент, который можно просмотреть или удалить; 
остальная часть - тело(сам стек).
Стек - рекурсивная структура данных. Может быть описан как конкатенация(склеивание, причем конкатенация!=слиянию) элемента типа Т и стека S. Для эффективной вычислимости необходим терминатор, ограничивающий глубину рекурсии описания, его роль играет пустой стек.

type СТЕК-Т = (ПУСТО | HЕПУСТОЙСТЕК)
type HЕПУСТОЙСТЕК = ( верхушка: Т; тело: СТЕК-Т)

Язык программирования со стоковой моделью вычисления. Форт. Используется обратная польская запись( постфиксная). (1+2)*4 = 1 2+ 4*.
Суть: 
если операнд, то записывается в стек
...
Сортировка и поиск**
14) Алгоритмы поиска (уметь написать на си)
    - 1.1) Линейный поиск
    - 1.2) Двоичный поиск
    - 1.3) Поиск в таблице
...
15) Алгоритмы сортировки (уметь написать на си)
