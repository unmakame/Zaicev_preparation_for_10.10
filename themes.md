...
Алгоритм Бойера-Мура является эффективным методом поиска подстроки в строке. Он основан на
сравнении символов образца и строки справа налево, а не слева направо, как в большинстве алгоритмов
поиска.
Шаги алгоритма Бойера-Мура:
1. Сгенерировать таблицу сдвигов. Для каждого символа алфавита определить максимально допустимое расстояние для сдвига образца вправо при 
несовпадении этого символа. Если символ вообще не
встречается в образце, то сдвиг производится на всю длину образца.
2. Инициализировать переменные: i - текущая позиция в строке (начиная с конца образца), j - текущая
позиция в образце.
3. Пока j > 0 и i < N (где N - длина строки):
- Сравнивать символ строки на позиции i и символ образца на позиции j.
- Если символы совпадают, уменьшить значения i и j.
- Если символы не совпадают:
- Произвести сдвиг образца вправо на значение, указанное в таблице сдвигов для данного символа
строки.
- Обновить значение i на i + d[s[i-1]], где d[s[i-1]] - значение сдвига для символа строки s[i-1].
4. Если j = 0, значит, обнаружено полное совпадение образца в строке.
Алгоритм Бойера-Мура эффективен, особенно в случае, когда образец содержит повторяющиеся или
небольшие фрагменты, которые можно использовать для быстрого сдвига. Этот метод может
значительно ускорить поиск по сравнению с методом Кнута-Морриса-Пратта (КМП), особенно в таких
ситуациях.
Важным элементом алгоритма Бойера-Мура является таблица сдвигов, которая определяет, на сколько
позиций можно сдвинуть образец при несовпадении символа. Это позволяет "пропускать" некоторые
части строки, которые гарантированно не содержат искомую подстроку .

## решение выражения ПОЛИЗ(Обратная польская) на С 
```
